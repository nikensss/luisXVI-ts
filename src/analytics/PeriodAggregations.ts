...
class PeriodAggregations {
  ...
  constructor(
    period: Period,
    name: string,
    data: PeriodAggregations[] | Aggregation[]
  ) {
    this._period = period;
    this._name = name;
    this._data = data;
  ...
  get aggregations(): Aggregation[] {
    ...
    throw new Error('[PeriodAggregations] Data is not Aggregation[]!');
  }
  ...
  hasAggregations(): boolean {
    return this.data.every(
      (d: PeriodAggregations | Aggregation) => d instanceof Aggregation
    );
  }

  hasPeriodAggregations(): boolean {
    return this.data.every(
      (d: PeriodAggregations | Aggregation) => d instanceof PeriodAggregations
    );
  }
  ...
  toTableColumn(): TableColumn {
    if (this.hasAggregations()) {
      ...
      return new TableColumn(
        this.name,
        this.totalSpan(),
        this.periodAggregations
      );
    }

    throw new Error(
      '[PeriodAggregations] Illegal State! Check type of elements in this.data!'
    );
  }

  getAggregatedMetricNames(): Set<Metric> {
    if (this.data.length === 0) {
      throw new Error('No metric has been aggregated!');
    }

    if (this.hasAggregations()) {
      return new Set<Metric>(this.aggregations.map(a => a.name));
    }

    return new Set<Metric>(
      this.periodAggregations.map(p => [...p.getAggregatedMetricNames()]).flat()
    );
  }

  getAggregatedMetric(metric: Metric): number[] {
    if (this.data.length === 0) {
      throw new Error('No metric has been aggregated!');
    }
    ...
    return this.periodAggregations
